...
- Set server-only `DATABASE_URL` (external PostgreSQL connection), `CLAIM_PROXY_SECRET` (same as Render) and `EARLY_BIRD_BACKEND_URL` (the existing Render API origin; see `../backend/EARLY_BIRD.md`).
...
2. Try the existing Render API for up to six seconds.
